...
from research.clean_walkforward_technical_hgb_excess_target import REPORT_DIR, account_metrics

RESEARCH_ID = "clean_shsz_conditional_rank_existing_exit_2019_2024_20260808"
SELECTED_TRADES = REPORT_DIR / "clean_shsz_conditional_risk_rank_2019_2024_20260808_trades.csv"


def simulate_existing_exit_2019_2024(
    selected: pd.DataFrame,
) -> tuple[list[dict], pd.DataFrame, pd.DataFrame]:
    """按冻结信号日调用既有 BacktestV2 退出与固定槽位逻辑。"""
    ...
    for _, row in selected.sort_values(
        ["trade_date", "prediction"], ascending=[True, False]
    ).iterrows():
    ...
    for signal_date, rows in sorted(by_signal_date.items()):
        ...
        for item in allowed:
            ...
            result = engine._simulate_trade(
                ...
                select_close=0.0,
                regime_max_hold=5,
                track_type="clean_shsz_conditional_risk_rank",
...
def run() -> dict:
    ...
    selected = pd.read_csv(SELECTED_TRADES, dtype={"trade_date": str})
    cohort = summarize(selected)
    exit_trades, base_curve, stress_curve = simulate_existing_exit_2019_2024(selected)
    ...
    result = {
        ...
        "exit_diagnostics": {
            ...
            "average_profit_after_fee_pct": (
                float(exit_frame["profit_after_fee"].mean()) if len(exit_frame) else None
            ),
            "exit_reasons": (
                exit_frame["exit_reason"].value_counts().to_dict()
                if "exit_reason" in exit_frame
                else {}
            ),
        },
        ...
        "note": "候选不变，仅复用项目正式短线退出路径；未使用2025信号日。",
    }
