...
PREREG = ROOT / "reports" / "research" / "prereg_post_earnings_pullback_20260808.json"
CANDIDATES = ROOT / "reports" / "research" / "post_earnings_pullback_candidates_20260808.csv"
REPORT = ROOT / "reports" / "research" / "post_earnings_pullback_validation_20260808.md"
FORBIDDEN_COLUMNS = {"ret_3d", "ret_5d", "ret_8d", "mfe_8d", "mae_8d", "opportunity_score", "opportunity_rank"}
SCORE_COLUMNS = {
    "netprofit_yoy", "roe", "debt_to_assets", "ret_5", "drawdown_20",
    "industry_rs_20", "days_since_announcement", "volume_ratio",
}
FROZEN_CONFIG = {"topn": 2, "rank_ridge": 100.0, "gate_ridge": 100.0, "gate_window_years": 99, "gate_quantile": 0.8, "cost": 0.25, "regime_mode": "all"}
...
def build_candidates(panel: pd.DataFrame, financial_events: pd.DataFrame, topn: int = 60) -> pd.DataFrame:
    ...
    numeric_columns = SCORE_COLUMNS | {
        "history_count", "pct_chg", "turnover_rate", "entry_gap_pct", "ret_60", "ret_20",
        "rsi_14", "ma_20", "ma_60", "close",
    }
    ...
    mask = (
        ...
        & work["drawdown_20"].between(3.0, 15.0)
        & work["rsi_14"].between(40.0, 68.0)
        & work["industry_rs_20"].ge(-2.0)
        & work["ma_20"].gt(work["ma_60"])
        & work["close"].ge(work["ma_20"] * 0.95)
    )
    work = work[mask].copy()
    if work.empty:
        return work
    ret5_quality = -(work["ret_5"] + 3.0).abs()
    drawdown_quality = -(work["drawdown_20"] - 7.0).abs()
    work["post_earnings_pullback_score"] = (
        _rank(work, work["netprofit_yoy"], True) * 0.20
        + _rank(work, work["roe"], True) * 0.15
        + _rank(work, work["debt_to_assets"], False) * 0.10
        + _rank(work, ret5_quality, True) * 0.15
        + _rank(work, drawdown_quality, True) * 0.10
        + _rank(work, work["industry_rs_20"], True) * 0.15
        + _rank(work, work["days_since_announcement"], False) * 0.10
        + _rank(work, work["volume_ratio"], False) * 0.05
    ).round(4)
    selected = (
        work.sort_values(["trade_date", "post_earnings_pullback_score", "ts_code"], ascending=[True, False, True])
        .groupby("trade_date", group_keys=False).head(topn)
        .drop_duplicates(["trade_date", "ts_code"]).copy()
    )
    selected["engine"] = "post_earnings_pullback"
    selected["engine_score"] = selected["post_earnings_pullback_score"]
...
def evaluate(path: Path, validation_allowed: bool) -> tuple[bool, pd.DataFrame]:
    ...
    train_pass = bool(train["trades"] >= 100 and train["avg_net"] > 0 and train["profit_factor"] > 1.10 and train["positive_years"] == train["years"] == 3)
    ...
    yearly = trades.assign(year=trades["trade_date"].astype(str).str[:4].astype(int)).groupby("year").apply(lambda group: pd.Series(_metrics(group)), include_groups=False).reset_index()
    ...
    strict_pass = bool(
        validation["trades"] >= 100 and validation["avg_net"] > 0 and validation["profit_factor"] > 1.10
        and validation["positive_years"] == validation["years"] == 3
        and recent["trades"] >= 40 and recent["avg_net"] > 0 and recent["profit_factor"] > 1.10
        and recent["positive_years"] == recent["years"] == 2 and ci_low > 0 and p_nonpositive < 0.05
        and len(validation_path) == 3 and (validation_path["return_pct"] > 0).all() and path_drawdown > -20.0
    )
    ...
    lines = [
        ...
        "- 所有财务记录均按公告日点时合并。", "- v1首次验证后不得改参，失败结果原样保留。",
