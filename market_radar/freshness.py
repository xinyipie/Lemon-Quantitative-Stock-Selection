...
POLICY_PROVIDERS = {"csrc_policy", "ndrc_policy", "miit_policy", "pbc_policy"}
POLICY_SOURCES = {"中国证监会", "国家发改委", "工业和信息化部", "中国人民银行"}
ANNOUNCEMENT_PROVIDERS = {"cninfo_announcements"}
ANNOUNCEMENT_SOURCES = {"巨潮资讯"}
...
def classify_news_time(value, now=None, record: dict | None = None) -> dict:
    """按媒体、公告和政策分别判定时效，价值判断仍交由后续 AI 完成。"""
    published = parse_datetime(value)
    reference = _now(now)
    information_type, fresh_hours, max_age_hours = news_freshness_window(record)
    ...
    if age_hours < -1:
        ...
        label = "政策观察期" if information_type == "policy" else "公告有效期" if information_type == "announcement" else "24小时内"
        status, eligible, factor, reason = "fresh", True, 1.0, f"发布时间可靠，处于{label}。"
    elif age_hours <= max_age_hours:
        label = "政策持续影响" if information_type == "policy" else "公告持续影响" if information_type == "announcement" else "持续影响"
        factor = 0.65 if information_type == "policy" else 0.5
        status, eligible, reason = "aging", True, f"发布时间仍在{label}窗口，具体价值由 AI 结合内容判断。"
    else:
        status, label, eligible, factor, reason = "stale", "已过期", False, 0.0, f"发布时间超过该类信息的{max_age_hours / 24:g}天窗口。"
...
def news_freshness_window(record: dict | None = None) -> tuple[str, float, float]:
    """返回信息类型、强时效窗口和最大有效窗口（小时）。"""
    item = record or {}
    ...
    text = " ".join((provider, source, event_type))
    if provider in POLICY_PROVIDERS or source in POLICY_SOURCES or any(word in event_type for word in ("政策", "监管")):
        return "policy", 72.0, 14 * 24.0
    if provider in ANNOUNCEMENT_PROVIDERS or source in ANNOUNCEMENT_SOURCES or "公告" in text:
        return "announcement", 24.0, 3 * 24.0
    return "media", NEWS_FRESH_HOURS, NEWS_MAX_AGE_HOURS
