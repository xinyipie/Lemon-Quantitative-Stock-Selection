...
def select_latest_result(results, label_contains='2025'):
    """按实际指标文件修改时间选择匹配区间的最新结果。"""
    candidates = [row for row in results if label_contains in str(row.get('label', ''))]
    candidates = candidates or list(results)
    if not candidates:
        raise ValueError("test_result.json 没有可分析的结果")

    def modified_time(row):
        raw = str(row.get('metrics_file', '') or '')
        paths = [raw, os.path.join('backtest_results', raw)]
        for candidate in paths:
            if candidate and os.path.exists(candidate):
                return os.path.getmtime(candidate)
        return -1.0

    return max(enumerate(candidates), key=lambda item: (modified_time(item[1]), item[0]))[1]


def trades_path_for_result(result):
    """把批量指标文件名映射为同次运行的交易 CSV。"""
    metrics_file = os.path.basename(str(result.get('metrics_file', '') or ''))
    stem, _ = os.path.splitext(metrics_file)
    return os.path.join('backtest_results', stem.replace('metrics_', 'trades_', 1) + '.csv')
...
def main():    # ── 读取最新的交易文件 ──
    ...
    # 取2025全年那段
    target = select_latest_result(result['results'], label_contains='2025')
    trades_file = trades_path_for_result(target)
    ...
    # ── 统一收益率字段名 ──
    profit_col = next((c for c in ['profit_after_fee', 'profit_pct', 'return_pct', 'pnl_pct', 'profit'] if c in df.columns), None)
