...
当前运行代码已更新至 `b543b866d5213adfb484ae694afad54f569255eb`：按用户确认恢复原行情中转，并完成真实接口查询。本文件保留初次全面修复的测试与发布记录；最终配置见 [修改总览](2026-09-07-change-summary.md) 和 [中转恢复记录](2026-09-07-legacy-relay-restoration.md)。
...
- 行情地址阻塞已解除：用户确认继续使用原中转 `http://14.nat0.cn:32817`，默认配置已恢复且生产查询成功；完整更新任务尚未因此重跑。
...
- 初次上线验收时行情有效日为 2026-09-04，未触发真实行情更新或外部 AI 生成。随后已恢复原 HTTP 中转并完成连接验证，Web 仍保持默认只读。
