...
## 2026-07-03 十年样本外数据底座进展

### 背景

用户明确担心 2025/2026 单年调参会过拟合，要求下载更多年份数据，用更长周期验证短线策略是否真的能筛出未来 3-5 天更容易上涨的股票。

### 改动

- 新增 `research/ten_year_cache_audit.py`：按 `daily` / `daily_basic` / `moneyflow` / `index_daily` 统计本地 parquet 覆盖率，并输出 `backtest_results/ten_year_cache_coverage.csv`。
- 新增 `research/ten_year_strategy_matrix.py`：固定策略矩阵和固定年度区间，用于后续做跨年份 OOS 对比，不在单一年份反复调参。
- `data_downloader.py` 新增 `--market-core`，用于下载 `daily` / `daily_basic` / `moneyflow` / `index_daily`，但跳过龙虎榜、融资融券、财务等慢字段。
- 实际下载先采用 `--core-only` 补股票核心数据，因为历史 `index_daily` 单独较慢，后续独立补齐。

### 当前覆盖率

| 数据集 | 2016 | 2017 | 2018 | 2019 | 2020 | 2021 | 2022 | 2023 | 2024 | 2025 | 2026H1 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| daily | 244 | 244 | 243 | 244 | 243 | 243 | 4 | 242 | 242 | 243 | 122 |
| daily_basic | 244 | 244 | 243 | 244 | 243 | 243 | 4 | 242 | 242 | 243 | 122 |
| moneyflow | 244 | 244 | 243 | 244 | 243 | 243 | 4 | 242 | 242 | 243 | 122 |
| index_daily | 0 | 0 | 0 | 0 | 0 | 0 | 4 | 242 | 242 | 243 | 122 |

### 结论

- 2016-2021 股票核心数据已补齐，可作为策略发现期/研究期样本。
- 2022 股票核心数据仍需补齐；2016-2021 的 `index_daily` 仍缺，不能直接用于依赖市场状态机或行业指数的最终稳健结论。
- 当前阶段是在补反过拟合的数据底座，不宣称策略已经找到；后续应先补齐数据，再跑固定矩阵，最后看跨年份胜率、收益、交易数和最差年份。
...
## 2026-06-03 选股门控实验：adaptive_quality_v6

### 背景

`adaptive_quality_v5` 在 2025 全年表现很好，但 2024H2 回测发现它误杀了部分好票。典型案例：

- `002249.SZ 大洋电机`：v2 买入收益 `+7.45%`，v5 因 `score >= 70` 且 `volume_ratio >= 3.2` 被过滤。

因此 v6 不再直接过滤所有“高分 + 量比过冲”，而是增加板块弱确认。
...
- 高分票放量不是一定危险；
- 如果板块支持不弱，允许保留；
- 只有“高分 + 放量过冲 + 板块偏弱”才认为是更高风险的追高。

### 2024H2 验证

命令：
```text
python test.py --scenario profile_v4_adaptive_quality_v2,profile_v4_adaptive_quality_v5,profile_v4_adaptive_quality_v6 --exit-profile baseline --start 20240701 --end 20241231 --label 2024H2_gate_v6
```

结果：

| 场景 | 笔数 | 胜率 | 总收益 | Alpha | Sharpe | MAE均值 | 窗口期末 |
|---|---:|---:|---:|---:|---:|---:|---:|
| profile_v4_adaptive_quality_v2 | 21 | 38.10% | +7.30% | -5.83% | +0.710 | -6.65% | -1.96% |
| profile_v4_adaptive_quality_v5 | 19 | 36.84% | +5.93% | -7.21% | +0.580 | -6.30% | -1.40% |
| profile_v4_adaptive_quality_v6 | 20 | 40.00% | +8.56% | -4.58% | +0.835 | -6.07% | -1.23% |

交易替换诊断：

| 对比 | 少买收益 | 多买收益 | 替换收益差 | 结论 |
|---|---:|---:|---:|---|
| v2 -> v6 | -3.53% | -0.07% | +3.46% | v6 比 v2 更好 |
| v5 -> v6 | 0.00% | +7.45% | +7.45% | v6 修复 v5 误杀 |

### 阶段结论

- v6 修复了 v5 在 2024H2 的核心问题。
- v6 相比 v2：收益、胜率、夏普、MAE、窗口期末均更好。
- v6 相比 v5：主要优势来自把 `002249.SZ 大洋电机` 这类“板块不弱的放量高分票”买回来。
- 下一步必须跑 2025 全年和 2026Q1：如果 v6 能保留 v5 在 2025 的大部分提升，同时不伤 Q1，则 v6 可以替代 v5 成为新的默认候选。

### 2025 全年验证

命令：
```text
python test.py --scenario profile_v4_adaptive_quality_v2,profile_v4_adaptive_quality_v5,profile_v4_adaptive_quality_v6 --exit-profile baseline --start 20250101 --end 20251231 --label 2025_gate_v6
```
...
| 场景 | 笔数 | 胜率 | 总收益 | Alpha | Sharpe | MAE均值 | 窗口期末 |
|---|---:|---:|---:|---:|---:|---:|---:|
| profile_v4_adaptive_quality_v2 | 154 | 44.16% | +51.40% | +30.21% | +1.526 | -4.94% | +1.17% |
| profile_v4_adaptive_quality_v5 | 152 | 45.39% | +62.03% | +40.84% | +1.753 | -4.97% | +1.39% |
| profile_v4_adaptive_quality_v6 | 154 | 45.45% | +61.01% | +39.82% | +1.740 | -4.96% | +1.40% |

交易替换诊断：

| 对比 | 少买收益 | 多买收益 | 替换收益差 | 结论 |
|---|---:|---:|---:|---|
| v2 -> v6 | -28.13% | -9.59% | +18.54% | v6 仍明显优于 v2 |
| v5 -> v6 | 0.00% | -2.08% | -2.08% | v6 比 v5 略少赚 |

补充观察：
- v6 相比 v5 多放回 2 笔交易，合计 `-2.08%`。
- 其中 `000503.SZ 国新健康` 亏 `-4.01%`，`002258.SZ 利尔化学` 赚 `+1.93%`。
- v6 比 v5 少赚约 `1.02%`，但修复了 2024H2 中 v5 的误杀问题。
...
## 2026-06-13 长线候选池实验：v18 market sync + Top3 alert cooldown

### 背景

波段/长线旧逻辑在组合回测中表现不稳定，且候选池过大。新的长线方向不再追求每日固定买入，而是建立“长线观察池”：

- 每日扫描市场；
- 只有满足严格市场同步与个股质量条件时才入池；
- 池内只做少量提醒，不做自动交易；
- 同一股票短期内不重复提醒，避免用户被噪音打扰。

### 已验证链路

```text
准入器：longterm_quality_lifecycle_v18_market_sync
候选池：longterm_pool_quality_audit.py
压缩层：longterm_pool_compression_audit.py
状态层：longterm_pool_state_audit.py
提醒层：longterm_pool_alert_audit.py
```
...
| 口径 | 样本/事件 | 80日均收益 | 80日胜率 | 跑赢沪深300 |
|---|---:|---:|---:|---:|
| v18 原始日频候选 | 347 | +10.29%（生命周期口径） | 58.88% | 59.54% |
| 每日Top5快照 | 266 | +9.56% | 58.62% | 60.54% |
| 每日Top3快照 | 209 | +9.47% | 58.05% | 59.51% |

2. 锁仓式压缩失败：

```text
entry-lock：推荐34条，80日均收益 +4.58%，跑赢 48.39%
```

原因：长线候选池是观察名单，不是自动买入锁仓；锁仓式会过早占坑，错过后续强票。

3. 周频固定推送失败：

| 方案 | 推荐数 | 80日均收益 | 胜率 | 跑赢沪深300 |
|---|---:|---:|---:|---:|
| weekly Top3 | 69 | +6.20% | 48.53% | 50.00% |
| weekly Top5 | 89 | +5.58% | 50.00% | 50.00% |

原因：固定周频会错过事件触发时点。长线可以日扫，但不必每日推送。

4. 提醒冷却期有效：

| 方案 | 原始入池事件 | 冷却后提醒 | 80日均收益 | 胜率 | 跑赢沪深300 |
|---|---:|---:|---:|---:|---:|
| Top3 + 80天冷却 | 186 | 125 | +12.90% | 66.94% | 65.29% |
| Top5 + 80天冷却 | 238 | 152 | +12.42% | 64.63% | 65.31% |

分阶段（Top3 + 80天冷却）：

| 阶段 | 提醒数 | 80日均收益 | 80日中位数 | 胜率 |
|---|---:|---:|---:|---:|
| 2024H2 | 55 | +16.88% | +7.90% | 65.45% |
| 2025H2 | 64 | +9.89% | +6.61% | 68.75% |
| 2026H1/Q1有效样本 | 2 | -0.07% | -0.07% | 样本太少，仅参考 |

### 当前结论

- 长线当前最成熟候选方案：`v18_market_sync + 每日Top3快照 + 同股80天提醒冷却`。
- 这是“长线观察池/提醒池”，不是自动交易策略。
- 不能采用周频固定推送，也不能采用锁仓式占坑。
- 不继续增加行业黑名单、个股小样本阈值或复杂权重，避免过拟合。
- 下一步应把该提醒层接入数据库状态表：`new / continue / downgraded_watch / removed / re_alert`，然后再考虑实盘报告展示。

## 2026-06-13 长线精英提醒层：elite_alert_v1

### 背景

用户反馈普通 Top3 + 80天冷却仍有 125 次提醒，作为长线高确信名单偏多。继续压缩时不再做复杂权重拟合，而是在已验证的 Top3 冷却提醒层上，使用少数有经济含义的硬条件：

```text
compression_score >= 80
industry_rs >= 8
drawdown_from_high between 7 and 15
```

含义：

- `compression_score >= 80`：通过行业、趋势、回撤、换手、重复出现等综合压缩层；
- `industry_rs >= 8`：必须处于明显强于市场/行业相对强势区域；
- `drawdown 7%-15%`：不是纯追高，也不是深跌弱修复，而是强趋势中的适度回撤。

### 结果

| 方案 | 提醒数 | 80日有效样本 | 80日均收益 | 80日中位数 | 胜率 | 跑赢沪深300 |
|---|---:|---:|---:|---:|---:|---:|
| Top3 elite + 80天冷却 | 35 | 33 | +26.68% | +15.99% | 84.85% | 90.91% |
| Top5 elite + 80天冷却 | 37 | 35 | +25.72% | +15.91% | 85.71% | 94.29% |
...
- 相邻阈值稳定：score 75/80、drawdown 上限 15/18、Top3/Top5 均能保持较高收益和胜率。
- 不依赖单一大牛：Top3 elite 剔除最大赢家后，80日均收益仍 +20.28%；剔除前三大赢家后仍 +17.44%。
- 不引入行业黑名单：亏损样本分散，暂不按行业做小样本剔除。
- 不采用周频固定推送：周频 Top3/Top5 明显变差。
- 不采用锁仓占坑：entry-lock 会错过后续强票。

### 当前判断

`elite_alert_v1` 是当前长线最高确信提醒层，适合一年只出少量提醒的用户体验。普通 `Top3 + 80天冷却` 可保留为观察层，`elite_alert_v1` 用于重点提示。

尚未定为自动实盘买入规则。下一步应做：

- 数据库状态落地：保存普通观察层与 elite 提醒层；
- 实盘报告展示：今日新增 elite、继续观察、移出；
- 后续再用 2026 后续数据做样本外验证。
...
- 新增 `research/ten_year_cache_audit.py`，输出 `backtest_results/ten_year_cache_coverage.csv`。
- 新增 `research/ten_year_strategy_matrix.py`，用于固定策略矩阵跨年份回测。
- 修改 `data_downloader.py`，增加 `--market-core`，同时采用 `--core-only` 分阶段补历史股票核心数据。
- 已完成下载：2016、2017、2018 的 `daily` / `daily_basic` / `moneyflow`。
- 已补齐：2026-06-26 到 2026-06-30 的 `index_daily`。

### 当前覆盖

| 年份 | daily | daily_basic | moneyflow | index_daily | 备注 |
|---|---:|---:|---:|---:|---|
| 2016 | 244 | 244 | 244 | 0 | 股票核心完整，历史指数待补 |
| 2017 | 244 | 244 | 244 | 0 | 股票核心完整，历史指数待补 |
| 2018 | 243 | 243 | 243 | 0 | 股票核心完整，历史指数待补 |
| 2023 | 242 | 242 | 242 | 242 | 已可完整验证 |
| 2024 | 242 | 242 | 242 | 242 | 已可完整验证 |
| 2025 | 243 | 243 | 243 | 243 | 已可完整验证 |
| 2026H1 | 122 | 122 | 122 | 122 | 已补齐到 2026-06-30 |

### 结论

- 先不要继续围绕 2025/2026 调参；新的研究主线是用 2016-2026H1 的跨周期样本做固定矩阵验证。
- 历史 `index_daily` 逐日逐指数下载太慢，应单独优化或只补关键指数，避免拖慢股票核心样本铺设。
- 下一步优先补 2019-2022 股票核心数据，再跑固定策略矩阵；涉及市场状态或行业指数因子的结论，要等历史指数补齐后再定稿。

## 2026-07-03 数据底座补齐完成记录

### 动作

- 补齐 2022 `daily` / `daily_basic` / `moneyflow`：`python data_downloader.py --start 20220101 --end 20221231 --core-only`。
- 新增 `research/fill_index_daily_range.py`：按区间批量补齐指数缓存；大盘指数走 `index_daily`，申万行业指数走 `sw_daily`，再统一写回 `data/cache/index_daily/YYYYMMDD.parquet`。
- 强制重写 2016-2026H1 指数缓存：`python research/fill_index_daily_range.py --start 20160101 --end 20260630 --force --pause 1.0`。

### 验证结果

| 数据集 | 2016 | 2017 | 2018 | 2019 | 2020 | 2021 | 2022 | 2023 | 2024 | 2025 | 2026H1 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| daily | 244 | 244 | 243 | 244 | 243 | 243 | 242 | 242 | 242 | 243 | 122 |
| daily_basic | 244 | 244 | 243 | 244 | 243 | 243 | 242 | 242 | 242 | 243 | 122 |
| moneyflow | 244 | 244 | 243 | 244 | 243 | 243 | 242 | 242 | 242 | 243 | 122 |
| index_daily | 244 | 244 | 243 | 244 | 243 | 243 | 242 | 242 | 242 | 243 | 122 |

指数行数质量审计：2016-2025 每个交易日均 `>=20` 行；2026H1 为 116/122 天 `>=20` 行，6 个 0 行日期同时也是 `daily` / `daily_basic` / `moneyflow` 0 行，分别是 `20260223`、`20260406`、`20260501`、`20260504`、`20260505`、`20260619`，应视为交易日历空壳/非交易日，而不是指数单独缺失。

### 结论

- 十年样本外验证的数据底座已从“2023-2026 为主”扩展到 2016-2026H1。
- 后续可以跑固定策略矩阵，但结论仍要按有效交易日、交易数、胜率、收益、最大回撤和最差年份一起看，不能只追某一年漂亮结果。
